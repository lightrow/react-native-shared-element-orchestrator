import React, { FC, ReactNode, useCallback, useEffect, useId, useMemo, useRef } from 'react';
...
const SharedTransitionScene: FC<ISharedElementSceneProps> = ({
    ...
    const { onSceneUpdated, onSceneDestroyed, onSceneActivated, onSceneDeactivated, activeScenesIds, transitions } =
        useSharedTransition();
    ...
    const onElementDestroyed = useCallback((elementId: ISharedTransitionElement['id']) => {
    ...
    const startTransition = transitions.find(transitions => transitions.start.sceneId === id)?.start;
    const endTransition = transitions.find(transitions => transitions.end.sceneId === id)?.end;
